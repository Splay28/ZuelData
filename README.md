...
このリポジトリは、:contentReference[oaicite:0]{index=0} 上で管理されている、データ閲覧・投稿・検索・ファイル管理を行うための Web アプリケーションです。

---

## 主な機能

アプリは :contentReference[oaicite:1]{index=1}で実装されており、主に次の機能を提供します。 :contentReference[oaicite:2]{index=2}
...
- お知らせ（Notice）
  ...
  - 詳細（`/notice/content`）
  - 投稿画面（`/notice/publish`）
  - 投稿（`/api/upload/notice`） :contentReference[oaicite:4]{index=4}
...
- タスク一覧（`/task/list`）
- 返信（`/task/reply`）
- タスク投稿 API（`/api/task/publish`） :contentReference[oaicite:5]{index=5}
...
- オンラインストレージ 画面（`/netdisk`）とアップロード（`/netdisk/upload`, `/api/upload/file`） :contentReference[oaicite:7]{index=7}

### 検証コード
- 検証コード（`/api/v_code`） :contentReference[oaicite:8]{index=8}

---

## ディレクトリ / ファイル構成

リポジトリ直下は主に次の構成です。 :contentReference[oaicite:9]{index=9}
.
